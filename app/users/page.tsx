...
import {
  ShieldCheck, LayoutGrid, LogOut,
  Loader2, Trash2, UserPlus, Eye, EyeOff,
  ShieldAlert, Users, User,
} from "lucide-react";
...
interface UserRow {
  ...
  role: "admin" | "user"; createdAt: string | null;
}
...
export default function UsersPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) { setFormError(json.error ?? "Failed to create user"); return; }
  ...
  if (!isAdmin) return (
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="text-center space-y-3">
        ...
        <Button variant="outline" size="sm" asChild><a href="/">← Back to Dashboard</a></Button>
      </div>
    </div>
  );
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-20 bg-card/80 backdrop-blur-sm border-b border-border flex items-center justify-between px-6 py-3.5">
          ...
          <Button id="create-user-btn" size="sm" className="gap-1.5"
        ...
        <div className="flex-1 p-6 space-y-5">
              ...
              <CardContent>
                <form onSubmit={handleCreate} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input id="new-username" placeholder="e.g. 12345678" value={form.username}
                      onChange={(e) => setForm((f) => ({ ...f, username: e.target.value }))} required className="h-9 text-sm" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-foreground">Email</label>
                    <Input id="new-email" type="email" placeholder="user@example.com" value={form.email}
                      onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} required className="h-9 text-sm" />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-foreground">Password</label>
                    <div className="relative">
                      <Input id="new-password" type={showPassword ? "text" : "password"} placeholder="Strong password"
                        value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} required className="h-9 text-sm pr-9" />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select id="new-role" value={form.role} onChange={(e) => setForm((f) => ({ ...f, role: e.target.value }))}
                      className={cn("w-full h-9 rounded-md border border-input bg-transparent px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring")}>
                      <option value="user">User</option>
                      <option value="admin">Admin</option>
                    </select>
                  </div>
                  {formError && (
                    <div className="sm:col-span-2 rounded-lg border border-destructive/30 bg-destructive/5 px-3.5 py-2.5 text-sm text-destructive">⚠ {formError}</div>
                  )}
          ...
          <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
            <Table>
              ...
              <TableBody>
                {loadingUsers ? (
                  <TableRow><TableCell colSpan={6} className="text-center py-10"><Loader2 className="size-5 animate-spin text-primary mx-auto" /></TableCell></TableRow>
                ) : users.length === 0 ? (
                  <TableRow><TableCell colSpan={6} className="text-center py-10 text-muted-foreground text-sm">No users found</TableCell></TableRow>
                ) : users.map((u, i) => (
                  <TableRow key={u.id} className="hover:bg-primary/5 transition-colors">
                    ...
                    <TableCell>
                      {u.role === "admin"
                        ? <Badge className="bg-primary/10 text-primary border-primary/20 text-xs font-semibold gap-1" variant="outline"><ShieldCheck className="size-3" /> Admin</Badge>
                        : <Badge className="bg-muted text-muted-foreground border-border text-xs font-semibold gap-1" variant="outline"><User className="size-3" /> User</Badge>}
                    </TableCell>
                    ...
                    <TableCell>
                      {u.id === user?.id
                        ...
                        : <Button variant="ghost" size="sm" className="h-7 px-2 text-muted-foreground hover:text-destructive hover:bg-destructive/5"
                            disabled={deletingId === u.id} onClick={() => handleDelete(u.id, u.username)}>
