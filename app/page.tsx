...
import { useUser, logout } from "@/app/components/UserProvider";
...
function CheckIcon() { return <Check className="size-3" />; }
...
export default function DashboardPage() {
  ...
  function handleClear() {
    setInputValue(""); setSearchedSabil(""); setHasSearched(false);
    setRows([]); setTotal(0); setError("");
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <aside className="hidden md:flex flex-col w-56 shrink-0 bg-card border-r border-border sticky top-0 h-screen">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          ...
          <a href="/users" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-20 bg-card/80 backdrop-blur-sm border-b border-border flex items-center justify-between gap-4 px-6 py-3.5">
          <div>
            <h1 className="text-base font-bold text-foreground tracking-tight">Mumineen Directory</h1>
            {hasSearched && !loading && (
              <p className="text-xs text-muted-foreground mt-0.5 animate-in fade-in duration-200">
                {total === 0 ? `No results for Sabil "${searchedSabil}"` : `${total} member${total !== 1 ? "s" : ""} in Sabil ${searchedSabil}`}
              </p>
            )}
          </div>
          {hasSearched && (
            <div className="flex items-center gap-2 animate-in fade-in slide-in-from-top-1 duration-200">
              <div className="relative">
                ...
                <Input ref={inputRef} id="sabil-search-header" type="number" placeholder="Sabil number…"
              ...
              <Button id="search-btn-header" size="sm" className="h-8 text-xs px-3"
                onClick={() => doSearch(inputValue)} disabled={loading || !inputValue.trim()}>
        ...
        {!hasSearched && (
          <div className="flex-1 flex items-center justify-center p-8">
            ...
            <div className="relative text-center max-w-md w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <h2 className="text-2xl font-bold text-foreground mb-2 tracking-tight">Search by Sabil</h2>
              ...
              <div className="flex items-center gap-2 max-w-sm mx-auto">
                <div className="relative flex-1">
                  ...
                  <Input ref={inputRef} id="sabil-search-hero" type="number" placeholder="e.g. 2"
                    value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                    autoFocus className="pl-9 h-11 text-base [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
                </div>
                <Button id="search-btn-hero" className="h-11 px-5 text-sm font-semibold"
                  onClick={() => doSearch(inputValue)} disabled={loading || !inputValue.trim()}>
        ...
        {error && (
          <div className="mx-6 mt-4 rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive flex items-center justify-between">
            <span>⚠ {error}</span>
            <Button variant="ghost" size="sm" className="h-7 text-xs text-destructive hover:text-destructive" onClick={() => doSearch(searchedSabil)}>Retry</Button>
          </div>
        )}
        ...
        {hasSearched && !loading && !error && rows.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center p-8 animate-in fade-in duration-200">
            ...
            <Button variant="outline" size="sm" onClick={handleClear} className="mt-1">Clear search</Button>
          </div>
        )}
                ...
                <TableBody>
                  {rows.map((r, i) => (
                    <TableRow key={r.id} className="hover:bg-primary/5 transition-colors">
                      ...
                      <TableCell>
                        {r.sector ? <Badge variant="secondary" className="text-primary bg-primary/10 border-primary/20 font-semibold text-xs">{r.sector}</Badge> : <span className="text-muted-foreground">—</span>}
                      </TableCell>
                      <TableCell className="text-muted-foreground text-xs">{r.subSector || "—"}</TableCell>
                      <TableCell className="text-xs text-foreground/70 max-w-[160px] truncate">{r.masool || "—"}</TableCell>
                      <TableCell className="text-xs text-foreground/70 max-w-[160px] truncate">{r.musaeed || "—"}</TableCell>
                      ...
                      <TableCell className="font-mono text-xs text-sky-600">
                        {String(r.mobileNo) !== "" && String(r.mobileNo) !== "[object Object]" ? String(r.mobileNo) : "—"}
                      </TableCell>
                      <TableCell>
                        {r.silatGiven ? (
